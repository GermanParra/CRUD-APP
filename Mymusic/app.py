...
from .spotify_data_access import search_albums, get_albums_data
from .spotify_data_access import get_albums_tracks_data, create_player_sources
...
def create_app():
    ...
    # Turn off verification when we request changes to db
    ...
    def search():
        ...
        return render_template('home.html',
                               Albums=albums,
                               Albums_count=albums_count,
                               S_albums_data=s_albums_data,
                               S_tracks_data=s_albums_tracks_data,
                               S_players=s_players
                               )
    ...
    def s_tracks():
        ...
        albums = Albums.query.all()
        return render_template('s_tracks.html',
                               Albums=albums,
                               Songs=s_album_tracks,
    ...
    def album_insert():
        ...
        for track in album_tracks[0]:
            song_row = Tracks(album_id=new_album.id,
                              name=track['name'],
        ...
        albums = Albums.query.all()
        albums_count = Albums.query.count()
        return render_template('album_insert.html',
                               New_album=new_album,
                               Albums=albums,
                               Albums_count=albums_count
                               )
    ...
    def edit_album():
        if "editing_album_id" in request.form:
            editing_album = Albums.query.filter_by(
                                        id=request.form["editing_album_id"]
                                                   ).first()
            # Quering DB to display table and row count on HTML
            albums = Albums.query.all()
            albums_count = Albums.query.count()
            return render_template('album_edit.html',
                                   Editing_album=editing_album,
                                   Albums=albums,
                                   Albums_count=albums_count
                                   )
        else:
    ...
    def album_delete():
        ...
        return render_template('album_delete.html',
                               Albums=albums,
    ...
    def song_delete():
        ...
        songs = Tracks.query.filter_by(album_id=song.album_id)
        return render_template('tracks_delete.html',
                               Songs=songs,
                               songs_count=songs_count,
                               album_info=album_info,
                               song_info=song
